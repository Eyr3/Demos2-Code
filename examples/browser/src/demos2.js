...
var DEMOS2 = function(ctx) {
    ...
    var DEMOS2 = {
        ...
        GenCRS(a1,a2,H2,U1,U2){

            var G2H2 = ctx.ECP2.generator();
            var G2U1 = ctx.ECP2.generator();
            var G2U2 = ctx.ECP2.generator();
            var s1,s2;
            ...
            if (a1 != null) {
                s1 = ctx.BIG.randomnum(q, a1);
                s1.toBytes(S1);
            } else {
                s1 = ctx.BIG.fromBytes(S1);
            }
            if (a2 != null) {
                s2 = ctx.BIG.randomnum(q, a2);
                s2.toBytes(S2);
            } else {
                s2 = ctx.BIG.fromBytes(S2);
            }

            G2H2 = ctx.PAIR.G2mul(G2H2, s1);
            G2H2.toBytes(H2);

            G2U1 = ctx.PAIR.G2mul(G2H2, s2);
            G2U1.toBytes(U1);

            G2U2 = ctx.PAIR.G2mul(G2H2, s2);
